// this file is auto generated by endoqa kotlin ffi, modify it with caution
package lib.quickjs

import java.lang.foreign.*
import java.lang.invoke.VarHandle

@JvmInline
public value class JSCFunctionListAliasEntry(
    public val `$mem`: MemorySegment,
) {
    public var name: Pointer<Byte>
        get() = nameHandle.get(this.`$mem`, 0L) as MemorySegment
        set(`value`) {
            nameHandle.set(this.`$mem`, 0L, value)
        }

    public var base: Int
        get() = baseHandle.get(this.`$mem`, 0L) as Int
        set(`value`) {
            baseHandle.set(this.`$mem`, 0L, value)
        }

    public constructor(gc: Boolean) : this(kotlin.run {
        require(gc) { "Do not call this if gc is not want" }
        Arena.ofAuto().allocate(layout)
    })

    public companion object {
        public val layout: StructLayout = MemoryLayout.structLayout(
            `$RuntimeHelper`.POINTER.withName("name"),
            ValueLayout.JAVA_INT.withName("base"),
            MemoryLayout.paddingLayout(4),
        ).withName("JSCFunctionListAliasEntry")

        @JvmField
        public val nameHandle: VarHandle =
            layout.varHandle(MemoryLayout.PathElement.groupElement("name"))

        @JvmField
        public val baseHandle: VarHandle =
            layout.varHandle(MemoryLayout.PathElement.groupElement("base"))

        @JvmStatic
        public fun allocate(alloc: SegmentAllocator): JSCFunctionListAliasEntry =
            JSCFunctionListAliasEntry(alloc.allocate(layout))
    }
}
